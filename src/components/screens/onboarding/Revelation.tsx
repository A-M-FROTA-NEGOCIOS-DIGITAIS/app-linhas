...
export function Revelation({ analysis, name, scanId, userId, onContinue }: Props) {
  ...
  const lines = [
    { text: `${name}, your hand is interesting.`, delay: 300 },
    { text: `You have a ${HAND_LABELS[analysis.hand_shape] ?? analysis.hand_shape}.`, delay: 0 },
    { text: `Your life line is ${lifeLine?.length ?? 'distinct'} and ${lifeLine?.depth ?? 'defined'}.`, delay: 0 },
    { text: `Your heart line ${heartLine?.characteristic ?? 'tells a story'}.`, delay: 0 },
    { text: `I need to tell you more.`, delay: 0, gold: true },
  ]
  ...
  return (
    <div className="h-full flex flex-col px-8 pt-16 pb-8"
      style={{ background: 'radial-gradient(ellipse at 50% 20%, rgba(201,169,97,0.05), transparent 55%)' }}>
