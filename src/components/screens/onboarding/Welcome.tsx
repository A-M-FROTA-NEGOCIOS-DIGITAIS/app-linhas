...
interface Props { name: string; gender?: Gender; onReadNow: () => void; onSetupPush: () => void }

export function Welcome({ name, gender, onReadNow, onSetupPush }: Props) {
  const { t } = useTranslation()
  const ctx = gender ?? 'neutral'
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center px-8 gap-8"
      ...
      <div className="text-center">
        ...
        <p className="text-xs text-text-muted tracking-widest uppercase mt-2">{t('welcome.label', { context: ctx })}</p>
      </div>
      ...
      <div className="text-center flex flex-col gap-3">
        <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 32, fontWeight: 300, letterSpacing: '-0.01em', color: 'var(--text-primary)', lineHeight: 1.2 }}>
          {t('welcome.title', { name, context: ctx })}
        </h2>
