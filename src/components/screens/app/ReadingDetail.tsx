...
export function ReadingDetail({ reading, onBack }: Props) {
  ...
  const TYPE_LABELS: Record<string, string> = {
    master: t('readingDetail.typeMaster'),
    daily: t('readingDetail.typeDaily'),
    themed: t('readingDetail.typeThemed'),
    compatibility: t('readingDetail.typeCompat'),
  }
  ...
  return (
    <div className="h-full flex flex-col">
      <TopBar title={TYPE_LABELS[reading.reading_type] ?? 'Reading'} onBack={onBack} />
      ...
      <div className="flex-1 scroll-area px-6 pt-4 pb-16">
        ...
        <div className="flex flex-col gap-6">
          {paragraphs.map((para, i) => {
            ...
            if (isHeader) {
              return <Eyebrow key={i} className="mt-2">{para.replace(/:$/, '')}</Eyebrow>
            }
            return (
              <p key={i} className="reading-text" style={{ fontFamily: 'var(--font-serif)', fontSize: 17, lineHeight: 1.75, color: 'var(--text-primary)', letterSpacing: '0.005em' }}>
                {para}
              </p>
            )
          })}
        ...
        <div className="mt-12 text-center">
          ...
          <p className="text-xs text-text-muted tracking-widest uppercase">{t('readingDetail.footer')}</p>
        </div>
      </div>
    </div>
  )
